...
% circular loop
plot(N_vec(3:end), Bmag(3:end)/(2*pi))
hold on;
% the magnetic field of a perfectly circular loop at the same current
B_ideal = 2*pi;
plot(N_vec(3:end), B_ideal_vec(3:end)/(2*pi));
xlabel("Data points (N sided polygons)");
ylabel("B-field magnitude normalized by 1/2*pi");
title("Magnetic fields for N sided polygons centered at origin");
legend("B-field of n side polygon", "B-field of circular loop")
figure 
...
error = abs(1 - Bmag/B_ideal) * 100;
error_cut_off = 5*ones(1,N_lim);
plot(N_vec(3:end), error(3:end));
hold on 
plot(N_vec(3:end), error_cut_off(3:end));
xlabel("Polygon of N sides");
ylabel("% error");
title("Error percentage of B-field of N-sided polygon to a circular loop");
legend("B-field approximation percentage comparison", " 5% cut off");
